...
class SingleWick():
    ...
    def get_middlewick_rate(self):
        ...
        for i in range(len(self.open) - 1):
            ...
            if self.close[i + 1] > self.open[i + 1]:
                para1 = (self.close[i + 1] - self.open[i + 1]) / self.low[i]
            # 下跌
            else:
                para1 = (self.open[i + 1] - self.close[i + 1]) / self.low[i]
    ...
    def get_upperwick_rate(self):
        ...
        for i in range(len(self.open) - 1):
            # 上涨
            if self.close[i + 1] > self.open[i + 1]:
                para2 = (self.high[i + 1] - self.close[i + 1]) / self.low[i]
                upperwick.append(para2)
            # 下跌
            else:
                para2 = (self.high[i + 1] - self.open[i + 1]) / self.low[i]
            upperwick.append(para2)
        return upperwick
    ...
    def get_lowerwick_rate(self):
        ...
        for i in range(len(self.open) - 1):
            # 上涨
            if self.close[i + 1] > self.open[i + 1]:
                para3 = (self.open[i + 1] - self.low[i + 1]) / self.low[i]
                lowerwick.append(para3)
            # 下跌
            else:
                para3 = (self.close[i + 1] - self.low[i + 1]) / self.low[i]
            lowerwick.append(para3)
        return lowerwick
    ...
    def get_singlewick(self):
        ...
        singlewick = list(zip(middlewick, upperwick, lowerwick, close_dif))
        for i in singlewick:
...
if __name__ == '__main__':
    global X
    X = []
    dirpath = r'C:\Users\lyzdsb\Desktop\finalPrograme'
    code_list = os.listdir(dirpath)
    for code in code_list:
        file_path = 'C:/Users/lyzdsb/Desktop/finalPrograme/' + code
        ...
        X = X + temp
    output = open(dirpath + 'raw_data_version2.pkl', 'wb')
